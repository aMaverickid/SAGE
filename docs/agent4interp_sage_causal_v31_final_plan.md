...
本文是工程版 lock 文档，凡未列入者一律推到 future work，本周不实现、不调研、不讨论。
...
| Trigger | SHES stagnation (K=2 refined streak, ε=0.05 score-gap) | v3 §18; main-80 间接支持 |
...
SHES.update_hes(hypothesis, test_history)
   ↓
SHES.should_trigger_commit(hypothesis, K=2, ε=0.05)
...
**为什么是 pre-update：** SHES 的证据只依赖刚完成的 input-side test 与 `ANALYZE_RESULT`，不需要等待下一次开放式 hypothesis rewrite。若先执行普通 `UPDATE_HYPOTHESIS`，LLM 可能已经把 hypothesis 标成 `CONFIRMED/REFUTED`，从而改变 active-hypothesis 集合并吞掉本应触发的 stagnation event。pre-update checkpoint 因此不是新 trigger，而是同一 SHES trigger 的工程时序落点：在继续消耗一次 refinement call 之前决定是否 commit。
